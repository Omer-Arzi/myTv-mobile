...
describe('HomeScreen — Watch Next post-mark-watched reconciliation', () => {
  ...
  it('marking the last currently-released episode of a still-RETURNING show: shows the "Caught up" success overlay, then removes the card — never "Completed"', async () => {
    mockGetHome.mockResolvedValue(baseHome());
    mockMarkEpisodeWatched.mockResolvedValue({
      watch: { id: 'watch-1', watchedAt: '2020-02-01T00:00:00.000Z', note: null, episode: episode() },
    ...
    // Brief success state (WatchNextCard's own success overlay, outcome-aware
    // label — see COMPLETION_COPY), correct copy, immediately.
    await waitFor(() => expect(getByText('Caught up')).toBeTruthy());
    ...
    await waitFor(() => expect(queryByText('Caught up')).toBeNull());
    expect(queryByText('S3E11')).toBeNull();
  });

  it('completing an ENDED series: shows the "Completed" success overlay, then removes the card — never "Caught up"', async () => {
    ...
    await waitFor(() => expect(queryByText('Completed')).toBeNull());
  });
